...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        relu1 = np.maximum(0, X.dot(W1) + b1) # ReLu第一次
        relu2 = np.maximum(0, relu1.dot(W2) + b2)# ReLu第二次
        scores = relu2.dot(W3) + b3
        ...
        # 反向传播计算
        p3 = np.exp(scores) / exp_sum
        p3[np.arange(N),y] += -1
        p3 /= N
        
        
        grads['W3'] = np.dot(relu2.T,p3)+reg*W3
        grads['b3'] = np.sum(p3,axis = 0)
        
        hid2 = np.dot(p3,W3.T)
        hid2[relu2<=0.00001] = 0
        
        grads['W2'] = np.dot(relu1.T,hid2) + reg*W2
        grads['b2'] = np.sum(hid2,axis=0)
        
        
        hid1 = np.dot(hid2,W2.T)
        hid1[relu2<=0.00001] = 0
        
        
        grads['W1'] = np.dot(X.T,hid1)+reg*W1
        grads['b1'] = np.sum(hid1,axis=0)
